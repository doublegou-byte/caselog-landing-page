...
这样即使 API 还没上线，`https://api.caselog.cn` 也能先返回一个证书正确的 `503`，后面再把这一段替换成真实反向代理即可。
...
- 只有 `gateway` 需要映射主机端口
- 当前实际部署可以先只启动 `gateway` 和 `web`
- `web` 不需要直接暴露到公网
- `gateway` 通过服务名 `web` 访问前端容器
- 等后端准备好后，再把 `api` 服务加回 `docker-compose.yml`
...
对于你当前这种“CVM 直连 + Docker 网关”的结构，推荐把方式 B 固定成：

- `acme.sh` 运行在服务器上
- DNS 验证使用 `DNSPod API`
- `acme.sh` 把证书安装到 `gateway` 挂载目录
- 续期成功后自动执行 `docker exec caselog-gateway nginx -s reload`
...
如果第一次签发时 DNS 传播慢，可以在 `--issue` 命令后额外加：

```bash
--dnssleep 180
```

这样以后证书续期不再依赖“腾讯云控制台下载证书压缩包再上传服务器”。
...
- HTTPS 由 `acme.sh + DNSPod API` 自动签发、自动续期、自动 reload 网关
...
- 推送形如 `v1.0.1` 的 Git Tag 时自动构建并推送镜像
...
- `1.0.1`、`1.0` 这类语义化版本标签：当使用 `v1.0.1` 这类 Git Tag 触发时生成
...
版本从 `1.0` 起步，当前建议的示例版本使用 Git Tag：
